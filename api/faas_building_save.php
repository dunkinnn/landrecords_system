...
function upsert($conn, $table, $data, $idField, $idValue) {
    ...
    if ($idValue) {
        ...
        if (!$stmt) throw new RuntimeException("Prepare failed for UPDATE $table: " . $conn->error);
        $stmt->bind_param($types . 'i', ...array_merge($values, [$idValue]));
        if (!$stmt->execute()) throw new RuntimeException("Execute failed for UPDATE $table: " . $stmt->error);
        $stmt->close();
        return (int) $idValue;
    }
    $placeholders = implode(', ', array_fill(0, count($columns), '?'));
    $stmt = $conn->prepare("INSERT INTO $table (" . implode(', ', $columns) . ") VALUES ($placeholders)");
    if (!$stmt) throw new RuntimeException("Prepare failed for INSERT $table: " . $conn->error);
    $stmt->bind_param($types, ...$values);
    if (!$stmt->execute()) throw new RuntimeException("Execute failed for INSERT $table: " . $stmt->error);
...
try {
    ...
    if (!$conn->query("DELETE FROM tbl_faas_building_items WHERE faas_building_id = " . (int) $buildingId)) {
        throw new RuntimeException('Failed to clear existing additional items: ' . $conn->error);
    }
    if (!empty($input['addl_item_desc'])) {
        ...
        if (!$stmt) throw new RuntimeException('Prepare failed for tbl_faas_building_items: ' . $conn->error);
        foreach ($input['addl_item_desc'] as $i => $desc) {
            ...
            if (!$stmt->execute()) throw new RuntimeException('Failed to insert additional item: ' . $stmt->error);
        }
        $stmt->close();
    }

    if (!$conn->query("DELETE FROM tbl_faas_building_superseded WHERE faas_building_id = " . (int) $buildingId)) {
        throw new RuntimeException('Failed to clear existing superseded records: ' . $conn->error);
    }
    if (!empty($input['superseded_pin'])) {
        $stmt = $conn->prepare("INSERT INTO tbl_faas_building_superseded
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)");
        if (!$stmt) throw new RuntimeException('Prepare failed for tbl_faas_building_superseded: ' . $conn->error);
        foreach ($input['superseded_pin'] as $i => $pin) {
            ...
            if (!$stmt->execute()) throw new RuntimeException('Failed to insert superseded record: ' . $stmt->error);
        }
        $stmt->close();
    }
